...
class SeatMap321(SeatMap):
    """A class representing a customized seat map for a specific aircraft model layout (Airbus A321-200)."""

    def __init__(self, master, passenger_id, flight_id):
        """Initialize the SeatMap321 object.
    ...
    def setup_layout(self):
        ...
        for i in range(1, 8):
            ...
            self.business_seat(grid_row=4, grid_column=i, aisle="a", seat_number=i)
            self.business_seat(grid_row=6, grid_column=i, aisle="c", seat_number=i)
            self.business_seat(grid_row=8, grid_column=i, aisle="d", seat_number=i)
            self.business_seat(grid_row=10, grid_column=i, aisle="f", seat_number=i)
            
            seat_row_number_label2 = tk.Label(self.seats_frame, text=i)
            seat_row_number_label2.grid(row=11, column=i)

        # Add labels for middle aisle
        label_aisle = tk.Label(self.seats_frame, text="")
        label_aisle.grid(row=7, column=0, padx=4, pady=4)

        # Add empty labels for space without seats (facilities)
        for i in range(8, 11):
            label_empty_business = tk.Label(self.seats_frame, text="")
            label_empty_business.grid(row=4, column=i, padx=4, pady=4)

        # Add labels for first section of seat row numbers and economy class seats
        # (last row has lower number of seats)
        for i in range(11, 23):
            seat_row_number_label1 = tk.Label(self.seats_frame, text=i - 3)
            seat_row_number_label1.grid(row=3, column=i)

            self.economy_seat(grid_row=5, grid_column=i, aisle="b", seat_number=i - 3)
            self.economy_seat(grid_row=6, grid_column=i, aisle="c", seat_number=i - 3)
            self.economy_seat(grid_row=8, grid_column=i, aisle="d", seat_number=i - 3)
            self.economy_seat(grid_row=9, grid_column=i, aisle="e", seat_number=i - 3)

            seat_row_number_label2 = tk.Label(self.seats_frame, text=i - 3)
            seat_row_number_label2.grid(row=11, column=i)

        for i in range(11, 22):
            self.economy_seat(grid_row=4, grid_column=i, aisle="a", seat_number=i - 3)
            self.economy_seat(grid_row=10, grid_column=i, aisle="f", seat_number=i - 3)

        # Add empty labels for space without seats (facilities)
        for i in range(23, 26):
            label_empty_economy = tk.Label(self.seats_frame, text="")
            label_empty_economy.grid(row=4, column=i, padx=4, pady=4)

        # Add labels for second section of seat row numbers and economy class seats
        for i in range(26, 38):
            seat_row_number_label1 = tk.Label(self.seats_frame, text=i - 6)
            seat_row_number_label1.grid(row=3, column=i)

            self.economy_seat(grid_row=4, grid_column=i, aisle="a", seat_number=i - 6)
            self.economy_seat(grid_row=5, grid_column=i, aisle="b", seat_number=i - 6)
            self.economy_seat(grid_row=6, grid_column=i, aisle="c", seat_number=i - 6)
            self.economy_seat(grid_row=8, grid_column=i, aisle="d", seat_number=i - 6)
            self.economy_seat(grid_row=9, grid_column=i, aisle="e", seat_number=i - 6)
            self.economy_seat(grid_row=10, grid_column=i, aisle="f", seat_number=i - 6)

            seat_row_number_label2 = tk.Label(self.seats_frame, text=i - 6)
            seat_row_number_label2.grid(row=11, column=i)
